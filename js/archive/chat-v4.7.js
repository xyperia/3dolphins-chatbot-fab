...
var Chat = {
	...
	enable_attachment: true,compatibility_mode: false,
		...
		if (rat !== null && rat !== undefined && rat.access_token !== undefined && rat.refresh_token !== undefined) {
			...
			if (!Chat.client || !Chat.client.connected) {
				...
				if (Chat.compatibility_mode) {
					...
					socket = new SockJS(Chat.url + '/webchat', null, {transports: ['xhr-streaming']});
				}
	...
	connectServer: function () {
		...
		if (Chat.webview || Chat.guestName || (cookieToken !== null && cookieToken !== '') || this.validateName(Chat.name, $("#warning-name")) && this.validateEmail(Chat.email, $("#warning-email")) && this.validatePhone(Chat.phone, $("#warning-phone"))) {
			...
			refreshAccessToken(Chat.url, Chat.client_id, Chat.client_secret, Chat.refresh_token, cookieToken, g_recaptcha, null, Chat.compatibility_mode);
		}	
	},
	subscribeIncoming: function (clientToken) {
		if (messageSubscription === undefined && Chat.client !== null) {
					...
					if (valid) {
						if (body.token !== null) {
							if (body.message === null && body.event === null) {
								...
								if (!Chat.webview) {
									getChatHistory(Chat.compatibility_mode);
								}
	...
	subscribeFeed: function () {
		if (Chat.session_id !== null && Chat.client !== null) {
	...
	buildChat: function (element) {
		if (element.childNodes.length === 0) {
			...
			var messages_box_html = '<textarea type=\"text\" class=\"message-input\" placeholder=\"' + Chat.type_placeholder + '\" disabled></textarea>[attachment_placeholder][voice_placeholder]' + '<button type=\"submit\" class=\"message-submit\"></button>';
	...
	buildError: function (element, message) {
		...
		if (message && typeof message.toLowerCase !== "undefined" && message.toLowerCase() !== 'retry') {
			...
			if (element !== null) {
				...
				if (message.toLowerCase().indexOf("connection") !== -1 && localStorage.getItem("messages") !== null) {
					if (!message.match("^{")) {
						...
						$('<div class=\"dolphin-warning-login-box\"><label id=\"warning-login\" class=\"dolphin-warning-login\">INITIATING...</label></div>').appendTo(element);						
					}
					setTimeout(function () {
						$(".dolphin-warning-login-box").remove();
					...
					if (Chat.state !== "icon") {
						if ($(".dolphin-chat").children().length === 0) {
							if (message.match("^{")) {
								$('<div class=\"dolphin-error-login-box\"><label id=\"error-login\" class=\"dolphin-error-login\">INITIATING...</label><br/>' + retry + '</div>').appendTo(element);
							} else {
							...
							if (message.match("^{")) {
								$('<div class=\"dolphin-error-login-box\" style=\"height:5%\"><label id=\"error-login\" class=\"dolphin-error-login\" style=\"font-size:12px;\">INITIATING...</label><br/>' + retry + '</div>').appendTo(element);	
							} else {
			...
			$(".dolphin-warning-login-box").remove();
			if (Chat.client !== null) {
				retry = '<button type=\"submit\" class=\"dolphin-message-retry\" onclick=\"Chat.reLogin()\">Retry</button>'
 ...
 */
function callbackOutgoingMessage(rat, ignoreMessageBox) {
...
function outgoingMessage() {
	...
	if (Chat.client !== null) {
		...
		refreshAccessToken(Chat.url, Chat.client_id, Chat.client_secret, Chat.refresh_token, Chat.token, Chat.g_recaptcha, callbackOutgoingMessage, Chat.compatibility_mode);
	} else {
		message = escapeSpecialChar(message);
		message = urlify(message);
...
function getAddress(latlng, msgObj) {
	geocoder.geocode({ 'location': latlng }, function (results, status) {
		if (status === 'OK') {
			...
			msgObj.latitude = undefined;
			msgObj.longitude = undefined;	
		} 
	});
}
...
function onCustomerTyping() {
	if (!Chat.iscustomertyping) {
		...
		refreshAccessToken(Chat.url, Chat.client_id, Chat.client_secret, Chat.refresh_token, Chat.token, Chat.g_recaptcha, callbackCustomerTyping, Chat.compatibility_mode);
	}
}
...
function viewForm(url) {
	...
	$('<button id="cancel" type="submit" class="message-submit"></button>').appendTo('.dolphin-message-box');
...
function getChatHistory(compatibility_mode) {
	if (Chat.enable_history && Chat.access_token && !isGettingChatHistory && Chat.token) {
 ...
 * @returns
 */
function refreshAccessToken(l, u, p, r, cookieToken, g_recaptcha, callback, compatibility_mode) {
	if (!Chat.webview) {
	...
	if (Chat.access_token && Chat.enable_queue && Chat.queue_text !== null && Chat.token) {
		...
		if (!compatibility_mode) {
			$.ajax({
				...
				data: 'token=' + Chat.token + '&access_token=' + Chat.access_token + "&accountId=" + Chat.email,
				success: function(data) {
			...
			queue_endpoint = queue_endpoint + '?access_token=' + Chat.access_token + "&accountId=" + Chat.email;
			$.ajax({
				url: queue_endpoint,
				type: GET,
				crossDomain: true,
